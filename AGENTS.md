...
HarnessAgent 是一个个人 Agent 应用，后端基于 Spring Boot 和 AgentScope Java，前端提供 React/Vite 个人工作台。系统面向单个 owner 长期运行的个人 Agent，不是企业多租户平台，也不是临时拼接聊天 SDK。

核心目标：

- 提供个人 Agent 的非流式和流式聊天 API。
- 保持 owner、Agent、会话和工作区之间的状态隔离。
- 支持带权限过滤、引用来源和无答案策略的 RAG。
- 对工具调用进行个人授权、参数校验、高风险确认、沙箱、幂等和最小审计。
- 覆盖生产运行时能力：持久化状态、工作区、快照、遥测、预算限制、fallback 和超时。
- 提供 Web 工作台，支持聊天、计划、文件、知识/记忆、工具、技能、Agent 配置和 trace/diagnostics。
...
|   +-- config/                     # Spring 配置属性
|   +-- console/                    # 个人工作台聚合和 diagnostics 视图服务，按 application/view 分层
|   +-- model/                      # 模型提供方抽象和实现
...
|   +-- release/                    # legacy 发布验收接口，默认个人路径不使用
|   +-- runtime/                    # owner/Agent/会话运行时上下文映射
|   +-- security/                   # owner 身份、个人授权、脱敏、Prompt 安全、Skill 审计，按 domain/application/persistence 分层
|   +-- session/                    # 聊天消息和会话存储，按 domain/persistence 分层
|   +-- tooling/                    # 工具注册、执行、权限、幂等和记录，按 domain/application/execution/persistence 分层
+-- src/main/resources
...
组件和 API 逻辑使用 Vitest，浏览器流程使用 Playwright。当前浏览器测试会 mock 后端 API 响应，覆盖聊天、计划、文件、知识/记忆、工具、技能、Agent 配置、trace/diagnostics 和响应式视图。
...
- 服务命令和持久化 key 中必须显式保留 `ownerScopeId`、`ownerId`、`agentId`、`sessionId`。legacy/migration 代码可以读取旧字段，但默认业务语义必须是 owner。
- 修改受治理流程时，不要绕过 `RuntimeContextFactory`、`AuthorizationService`、`DataPermissionService`、`PromptInjectionGuard`、`BudgetLimiter` 或审计服务。
- 密钥必须来自环境变量或 Spring 配置占位符。不要硬编码 API key、token、密码或 owner 凭据。
- 新增日志必须使用 SLF4J 参数化日志和 allowlist 字段。可以记录 `ownerScopeId`、`agentId`、状态、原因码、数量和耗时；`ownerId`、`sessionId`、`idempotencyKey` 等外部输入需 hash 或摘要；不要记录原始 prompt、query、工具参数、工具结果、DSN、Redis URI、snapshot 内容或 workspace 文件内容。

### 个人 Agent 约束

- 保持运行时隔离：
  - `ownerScopeId`、`ownerId`、`agentId`、`sessionId` 必须贯穿聊天、RAG、工具、遥测、审计和生产状态链路。
  - 跨 owner 访问默认拒绝。
- RAG 回答必须遵守 owner、agent、source visibility 和个人授权。没有可访问证据时，保留无答案行为，不要编造引用。
- 工具执行必须同时保留策略检查、参数校验、高风险确认、沙箱、幂等和最小审计记录。
...
- 工作台是工作型个人 Agent 界面，应保持紧凑、清晰、可扫描，不要做成营销页。
...
- 本地身份面板只展示 owner 和当前 Agent。生产身份必须来自可信本地或宿主基础设施，不能来自任意浏览器状态。
...
- 修改 diagnostics、运行时配置或回滚预期时，同步更新 `docs/`。
...
- 优先选择最小但完整的改动，同时保持个人授权、工具确认、RAG 可见性、状态恢复和最小审计不被削弱。
