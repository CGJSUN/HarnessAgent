...
| Permission System | https://java.agentscope.io/v2/zh/docs/building-blocks/permission-system.html | v2 / 2026-06-09 | `personal-tooling-hitl`、`personal-skill-integration` | 部分覆盖：工具权限已有 allow/deny/confirm 三态执行语义，个人 HITL pause 可持久化并支持确认、拒绝和改参恢复；可撤销授权和技能权限待实现 | 工具权限、HITL pending、改参恢复和 Console/API 契约测试已补；技能权限测试待补 |
