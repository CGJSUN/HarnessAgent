...
- `tenantId` 仅作为迁移、回滚和归档兼容列存在；个人版默认公共契约不要求用户理解或输入企业租户，AgentScope-facing `runtimeUserId` 派生为 `owner:<ownerId>`。
...
| Release API | `ReadinessController`、`PersonalReadinessService`、`ReadinessCheck` | 阶段门禁、生产治理 gate、回滚动作清单 | 个人版发布验收改为功能清单和手动 smoke；旧 `/api/release/**` 可继续用于诊断，但不代表个人版完整覆盖 |
| 运行时上下文 | `RuntimeContextFactory`、`RuntimeContextScope`、`TenantStateKeyStrategy`、Agent state store | tenant/user/agent/session 复合 key | 个人主路径使用 owner/agent/session；旧 key 通过 `TenantStateKeyStrategy` 仅用于迁移测试和兼容读取，runtimeUserId 派生为 `owner:<ownerId>` |
