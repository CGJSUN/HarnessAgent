...
import type { ConsoleActivityResult, CostUsageReport, OperationalMetricSummary } from "../api/types";
import {
  AccessDenied,
  EmptyState,
  ErrorState,
...
export function AuditWorkspace({ api }: { api: ApiClient }) {
  ...
  const [result, setResult] = useState<ConsoleActivityResult | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [accessDenied, setAccessDenied] = useState<string | null>(null);
  ...
  async function search(event?: FormEvent) {
    ...
    setAccessDenied(null);
    try {
      setResult(await api.recordActivity({ targetUserId, sessionId, resourceId, action, from, to }));
    } catch (caught) {
      handleApiError(caught, setError, setAccessDenied);
    } finally {
      setLoading(false);
  ...
  const count = (result?.toolActivity.length ?? 0) + (result?.securityActivity.length ?? 0);
  ...
  return (
    <section className="main-panel">
      ...
      {loading ? <LoadingState /> : null}
      {count === 0 ? <EmptyState title="No activity rows" /> : null}
      {result?.toolActivity.map(record => (
      ...
      {result?.securityActivity.map(record => (
        <article className="activity-row" key={record.id}>
          <div>
