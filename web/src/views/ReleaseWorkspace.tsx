...
import type { EndToEndAcceptanceReport, PhaseGate, ReleaseScenario, RollbackAction } from "../api/types";
import {
  AccessDenied,
  EmptyState,
  ErrorState,
...
export function ReleaseWorkspace({ api }: { api: ApiClient }) {
  ...
  const [phaseGates, setPhaseGates] = useState<PhaseGate[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [nextScenario, nextPhaseGates, nextRollback, nextAcceptance] = await Promise.all([
        api.releaseScenario(),
        api.phaseGates(),
        api.rollbackActions(),
        api.acceptance()
